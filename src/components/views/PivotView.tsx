...
const PivotView: React.FC<PivotViewProps> = ({
  ...
  const numericFields = [{
    ...
    key: 'totalNonPaid',
    label: 'Total Non-Paid'
  }, {
    key: 'classAverageIncludingEmpty',
    label: 'Class Average (All)'
  }, {
    key: 'classAverageExcludingEmpty',
    label: 'Class Average (Non-Empty)'
  }];
  const displayFields = [{
    key: 'cleanedClass',
    label: 'Class Type'
  ...
  const pivotData = useMemo(() => {
    ...
    const rowValues = Array.from(new Set(data.map(item => String(item[rowField])))).sort((a, b) => {
    ...
    const columnValues = Array.from(new Set(data.map(item => String(item[columnField])))).sort();
        ...
        case 'average':
          {
            ...
            pivotTable[rowValue][colValue] = pivotTable[rowValue][`${colValue}_total`] / pivotTable[rowValue][`${colValue}_count`];
            break;
          }
        case 'count':
  ...
  return <div className="p-6 bg-gradient-to-br from-purple-500 to-indigo-900 dark:from-gray-900 dark:to-gray-800">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
            <div className="space-y-2">
              ...
              <Select value={rowField as string} onValueChange={value => setRowField(value as keyof ProcessedData)}>
                ...
                <SelectContent>
                  {displayFields.map(field => <SelectItem key={field.key as string} value={field.key as string}>
                      {field.label}
                    </SelectItem>)}
                </SelectContent>
              </Select>
            ...
            <div className="space-y-2">
              ...
              <Select value={valueField as string} onValueChange={value => setValueField(value as keyof ProcessedData)}>
                ...
                <SelectContent>
                  {numericFields.map(field => <SelectItem key={field.key as string} value={field.key as string}>
                      {field.label}
                    </SelectItem>)}
                </SelectContent>
              </Select>
            ...
            <div className="space-y-2">
              ...
              <Select value={aggregationMethod} onValueChange={value => setAggregationMethod(value as AggregationMethod)}>
        ...
        <Table>
          <TableHeader className="bg-indigo-50 dark:bg-indigo-900/30">
            <TableRow>
              ...
              {pivotData.columnValues.map(colValue => <TableHead key={colValue} className="text-gray-800 dark:text-indigo-300 font-semibold px-8 py-2 text-center">
                  {colValue}
                </TableHead>)}
          ...
          <TableBody>
            {pivotData.rowValues.map(rowValue => <TableRow key={rowValue} className="hover:bg-indigo-50/50 dark:hover:bg-indigo-900/20">
              ...
              return <TableCell key={colValue} className="text-center">
                      {formatValue(value)}
                    </TableCell>;
            })}
            ...
            <TableRow className="bg-indigo-50 dark:bg-indigo-900/30 font-bold">
              ...
              {pivotData.columnValues.map(colValue => <TableCell key={colValue} className="text-center text-gray-900 text-bold dark:text-indigo-300">
