...
import { Users, Calendar, MapPin, Clock, User, IndianRupee, Calendar as CalendarIcon } from 'lucide-react';
...
const GridView: React.FC<GridViewProps> = ({
  ...
  const generateAvatarColor = (name: string) => {
    ...
    const colors = ['bg-blue-500', 'bg-indigo-500', 'bg-purple-500', 'bg-pink-500', 'bg-red-500', 'bg-orange-500', 'bg-amber-500', 'bg-yellow-500', 'bg-lime-500', 'bg-green-500', 'bg-emerald-500', 'bg-teal-500', 'bg-cyan-500', 'bg-sky-500'];
    return colors[Math.abs(hash) % colors.length];
  };
  return <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 p-4 bg-gradient-to-br from-purple-50 to-indigo-50 dark:from-gray-900 dark:to-gray-800">
      ...
      return <Card key={index} className="overflow-hidden transition-all hover:shadow-lg border-indigo-100 dark:border-indigo-900 bg-white dark:bg-gray-900">
              <CardHeader className="bg-gradient-to-r from-indigo-800 to-purple-900 dark:from-indigo-800 dark:to-purple-900 text-white rounded-lg py-3">
                <div className="flex justify-between items-start">
                  <Badge variant="outline" className="mb-2 bg-white/20 text-white border-white/30">
                    {item.period}
                  </Badge>
                  <Badge variant={parseInt(item.classAverageIncludingEmpty) > 10 ? "default" : "secondary"} className="flex items-center gap-1 bg-white/20 text-white hover:bg-white/30 border-white/30">
                    <Users className="h-3 w-3" />
                    {item.classAverageIncludingEmpty}
                  </Badge>
                </div>
                <CardTitle className="text-lg font-bold line-clamp-1 py-0">{item.cleanedClass}</CardTitle>
              </CardHeader>
                ...
                <div className="grid grid-cols-2 gap-y-3">
                  ...
                  <div className="flex items-center text-sm gap-1 col-span-2 mt-1">
                    <Avatar className="h-6 w-6">
                      {avatarUrl ? <AvatarImage src={avatarUrl} alt={item.teacherName} /> : <AvatarFallback className={`text-xs text-white ${avatarColor}`}>
                          {teacherInitials}
                        </AvatarFallback>}
                    </Avatar>
